<?php
// Before doing anything check to make sure the environment is not the live-backup
if ($_ENV['PANTHEON_ENVIRONMENT'] == 'live-backup') {
  exit("Don't send a message to slack for live-backup environment updates");
}
...
function _slack_notification($slack_url, $channel, $username, $text, $attachment, $alwaysShowText = false)
{
  ...
  // Watch for messages with `terminus workflow:watch SITE_ID`
  print("\n==== Posting to Slack ====\n");
  $result = curl_exec($ch);
  print("RESULT: $result");
